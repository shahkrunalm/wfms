...
public class DesignationController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		this.designation = (Designation) this.context.getBean("designation");
		
		if(action.equals(Constants.ADD)){
			final String designationName = request.getParameter("designationName");
			this.designation.setDesignationName(designationName);
			this.designation.setStatus(Constants.ACTIVE);
			this.designationDao.save(designation);
			
			// Populating active designation list
			getServletContext().setAttribute("activeDesignationList", this.designationDao.getListByCriteria(this.designation, "designationName", 1));
			
			response.sendRedirect(request.getContextPath() + "/save-successfully.jsp?entity=Designation");
		}else if(action.equals(Constants.VIEW)){
			final String status = request.getParameter("status");
			this.designationList = this.designationDao.getListByCriteria(this.designation, "designationName", Integer.parseInt(status));
			request.setAttribute("designationList", this.designationList);
			request.getRequestDispatcher("view-designation-list.jsp").forward(request, response);
			...
			final boolean isDesignationAvailable = this.designationDao.isExists(this.designation, "designationName", designationName);
